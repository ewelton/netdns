...
class DNSBuffer(Buffer):
    ...
    def encode_name(self,name):
        ...
        if not isinstance(name,NodeName):
            name = NodeName(name)
        if len(name) > 253:
            raise NodeNameError("Domain label too long: %r" % name)
        ...
        while name:
            if tuple(name) in self.names:
                ...
                element = name.pop(0)
                if len(element) > 63:
                    raise NodeNameError("Label component too long: %r" % element)
...
class DNSQuestion(object):
    ...
    def set_qname(self,qname):
        if isinstance(qname,NodeName):
            self._qname = qname
        else:
            self._qname = NodeName(qname)
...
class RR(object):
    ...
    def set_rname(self,rname):
        if isinstance(rname,NodeName):
            self._rname = rname
        else:
            self._rname = NodeName(rname)
...
class MX(RD):
    ...
    def set_label(self,label):
        if isinstance(label,NodeName):
            self._label = label
        else:
            self._label = NodeName(label)
...
class SOA(RD):
    ...
    def set_mname(self,mname):
        if isinstance(mname,NodeName):
            self._mname = mname
        else:
            self._mname = NodeName(mname)
...
class SRV(RD):
    ...
    def set_target(self,target):
        if isinstance(target,NodeName):
            self._target = target
        else:
            self._target = NodeName(target)
...
class NAPTR(RD):
    ...
    def set_replacement(self,replacement):
        if isinstance(replacement,NodeName):
            self._replacement = replacement
        else:
            self._replacement = NodeName(replacement)
